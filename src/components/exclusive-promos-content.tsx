...
interface ExclusivePromoLayout {
  [key: string]: string | number | boolean | null | undefined;
}

interface ExclusivePromosContentProps {
  searchTerm?: string;
}

export default function ExclusivePromosContent({ searchTerm: externalSearchTerm = '' }: ExclusivePromosContentProps) {
  const [data, setData] = useState<ExclusivePromoLayout[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [sortColumn, setSortColumn] = useState<string>('Brands');
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetch('/api/exclusive-promos');
        if (!response.ok) {
  ...
  return (
    <div className="container mx-auto py-10">
      <h1 className="text-3xl font-bold mb-6 text-center">Exclusive Promos</h1>
